...
<article id="content">
 ...
 <br></br> Hi, <font size="5" color="green"><%=session.getAttribute("un")%>...</font><br></br>
                                    ...
                                    <center> 
                                        <h2>Group Members</h2>
                                        <table align="center" cellspacing="10" bgcolor="white" width="50%" style="border-style: solid solid;border-color: #ff7500 ;">
    
     <tr height="38"><td ><center>S.No</center></td> <td ><center>Name</center></td> <td ><center>UserName</center></td>
 <td ><center>Email</center></td></tr>
    ...
    DB Db=new DB(); 
    int i=0;
    ResultSet rs=Db.Select("select * from reg where S_Name='"+session.getAttribute("Sn")+"' and Un!='"+session.getAttribute("un")+"'");
    while(rs.next())
    {
        i++;
    %>
<tr><td ><center><%=i%></center></td><td ><center><%=rs.getString("Name")%></center></td><td ><center><%=rs.getString("Un")%></center></td><td ><center><%=rs.getString("Email")%></center></td><td><center> <a href="Download_3.jsp?name=<%=rs.getString("Un")%>#!/page_SERVICES">Choose</a></center></td>
