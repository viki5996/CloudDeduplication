...
<nav class="menu">
  <ul id="menu">
   <li class="nav1">
    ...
    <% if(String.valueOf(session.getAttribute("Sn")).compareToIgnoreCase("Not")!=0) {%>
                                <li><a href="Groupmembers.jsp"><b>Group Members</b></a></li>
                                <%}
                                else
                                {
                                %>
...
<article id="content">
  ...
  <ul>
    ...
    DB Db=new DB(); 
    int i=0;
    ResultSet rs=Db.Select("SELECT count(Fid),Filename,Size,sum(Cost) FROM download where S_Name='"+session.getAttribute("Sn")+"' and Un='"+session.getAttribute("un")+"' group by Fid");
    while(rs.next())
    {
        i++;
    %>
