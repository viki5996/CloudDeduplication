...
<article id="content">
  ...
  <ul>
   ...
   <br></br> Hi, <font size="5" color="green"><%=session.getAttribute("un")%>...</font><br></br>
     ...
     <tr><td><center>S.No</center></td> <td><center>Name</center></td> <td><center>UserName</center></td>
 <td><center>Email</center></td><td><center>Scheme</center></td><td><center>Group Name</center></td></tr>
    <%
    DB Db=new DB(); 
    int i=0;
    ResultSet rs=Db.Select("select * from reg order by Scheme ");
    while(rs.next())
    {
        i++;
    %>
<tr><td ><center><%=i%></center></td><td ><center><%=rs.getString("Name")%></center></td><td ><center><%=rs.getString("Un")%></center></td><td ><center><%=rs.getString("Email")%></center></td>
<td ><center><%=rs.getString("Scheme")%></center></td><td ><center><%=rs.getString("S_Name")%></center></td>
</tr>
